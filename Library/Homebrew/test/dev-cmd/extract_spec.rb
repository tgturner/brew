describe "brew extract", :integration_test do
  ...
  it "retrieves the specified version of formula, defaulting to most recent with from argument" do
    path = Tap::TAP_DIRECTORY/"homebrew/homebrew-foo"
    ...
    expect { brew "extract", "testball", target.name, "--from=tgturner/test" }
      .to be_a_success
    ...
    expect { brew "extract", "testball", target.name, "--version=0.1", "--from=tgturner/test" }
      .to be_a_success
